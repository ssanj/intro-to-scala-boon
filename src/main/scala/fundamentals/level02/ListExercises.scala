...
object ListExercises {
  ...
  def min(nums: List[Int]): Int =
    nums match {
      ...
      case head :: tail => tail.foldLeft(head)((acc, v) => if (acc < v) acc else v)
    }
  ...
  def youngestPerson(persons: List[Person]): Person = persons match {
    ...
    case h :: t => t.foldLeft(h)((acc, v) => if (acc.age <= v.age) acc else v)
  }
  ...
  def personWithIndex(people: List[Person]): List[(Person, Int)] = {
    people.zipWithIndex.map { case (p, i) => (p, i + 1) }
  }
  ...
  def showEveryNthPerson(n: Int, persons: List[Person]): List[String] = {
    if (n <= 0) persons.map(showPerson1)
    else if (n > persons.length) Nil
    else {
      personWithIndex(persons).filter(_._2 % n == 0).map { case (p, _) => showPerson1(p) }
    }
  }
